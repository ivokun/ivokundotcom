...
const ALLOWED_MIME_TYPES = [
  ...
  'image/gif',
  'image/webp',
  // NOTE: SVG is intentionally excluded. SVG files can contain embedded
  // <script> tags and event handlers, creating a stored XSS vector.
  // Since uploads go directly to R2 via presigned URL, the server never
  // sees the file content during upload. SVG sanitization would need to
  // be added to the media processing pipeline before re-enabling.
  // To re-enable: add sanitization in confirmUpload processing step
  // using a library like DOMPurify or svg-sanitizer.
  // 'image/svg+xml',
  'video/mp4',
  'video/webm',
  'application/pdf',
